...
All of the below commands are intended to be executed on the remote TPU VM as deployed via the `create_tpu_vm.sh` script. If you have not done so already, `ssh` into it using `gcloud alpha compute tpus tpu-vm ssh --zone europe-west4-a atc24-ae-${USER}`
...
6. **Run ResNet50 experiment**. Run the following command to exectue the experiments for the ResNet in Fig 8. `python run_fig8.py ResNet50_v2-8`. More concretely this script does the following:
...
7. **Run RetinaNet experiment**. Run the following command to exectue the experiments for the RetinaNet in Fig 8. `python run_fig8.py retina`. It runs the same experiments as for ResNet.
8. **Tear down the cluster**. Please make sure to execute `./manage_cluster.sh stop` to tear down the cluster. All statuses should show a green `[OK]`.
