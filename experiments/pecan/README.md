...
6. **Run Hellow World example**. Run the following command to execute a short ResNet run to check everything is working correctly. `python run_fig8.py short` In this run all workers are used at all times and a single bar is produced in the plot `plots/Getting_started.pdf` (see our Reference Results below).
