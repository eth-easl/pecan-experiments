...
from official.vision.detection import exp_utils
...
flags.DEFINE_string(
    'dispatcher_ip', "otmraz-cachew-dispatcher-xxx", 'Dispatcher IP')
...
def run_executor(params,
  ...
  if mode == 'train':
    ...
    if FLAGS.no_model:
      ...
      dataset = train_input_fn.__call__()
...
def main(argv):
  ...
  if DISPATCHER_IP=='None':
    DISPATCHER_IP=None
  DISP_IP = exp_utils.get_disp_ip(DISPATCHER_IP)
  print("Dispatcher name is: " + str(DISP_IP))

  #spawn_local_worker(FLAGS.local_workers, f"{flags.FLAGS.dispatcher_ip}:31000")
  workers = tf.data.experimental.service.spawn_loc_workers(workers=FLAGS.local_workers, dispatcher=str(DISP_IP)+":31000")
  run()
