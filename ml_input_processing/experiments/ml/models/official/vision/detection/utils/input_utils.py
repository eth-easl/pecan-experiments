...
def normalize_image(image,
  ...
  offset = tf.constant(offset) #, dtype=image.dtype)
  offset = tf.expand_dims(offset, axis=0)
  offset = tf.expand_dims(offset, axis=0)

  #offset = tf.image.convert_image_dtype(offset, dtype=image.dtype)

  image -= offset

  scale = tf.constant(scale) #, dtype=image.dtype)
  scale = tf.expand_dims(scale, axis=0)
  scale = tf.expand_dims(scale, axis=0)

  #scale = tf.image.convert_image_dtype(scale, dtype=image.dtype)
