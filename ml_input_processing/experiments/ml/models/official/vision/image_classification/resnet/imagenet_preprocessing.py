...
# This should automatically find the dispatcher name
...
def process_record_dataset(dataset,
  ...
  if DISPATCHER_IP is not None and is_training is True and DISTRIBUTE_CHOICE == 1:
  ...
  # Only for a test
  #dataset = dataset.take(1024).cache().repeat()

  cores=multiprocessing.cpu_count()
  dataset = dataset.map(_decode_and_crop, tf.data.experimental.AUTOTUNE) #, keep_position=True)

  rads = 5 * math.pi / 180

  #dataset = dataset.map(lambda image, label: (tf.image.random_flip_left_right(image), label), tf.data.experimental.AUTOTUNE)

  #dataset = dataset.map(lambda image, label: (tfa.image.shear_x(tf.image.random_flip_left_right(image), level=random.uniform(-0.1, 0.1), replace=0), label), tf.data.experimental.AUTOTUNE)
  ...
  #dataset = dataset.map(lambda image, label: (tf.image.random_flip_left_right(image), label), tf.data.experimental.AUTOTUNE) # , keep_position=True)
  #rads = 5 * math.pi / 180
  #dataset = dataset.map(lambda image, label: (tfa.image.rotate(image, angles=random.uniform(0, rads)), label), tf.data.experimental.AUTOTUNE) #, keep_position=True)
  #dataset = dataset.map(lambda image, label: (tfa.image.shear_x(image, level=random.uniform(-0.1, 0.1), replace=0), label), tf.data.experimental.AUTOTUNE) #, keep_position=True)
  dataset = dataset.map(lambda image, label: (_resize_image(image, DEFAULT_IMAGE_SIZE, DEFAULT_IMAGE_SIZE), label), tf.data.experimental.AUTOTUNE, keep_position=USE_AUTOORDER==False)
  #dataset = dataset.map(lambda image, label: (tfa.image.shear_x(tfa.image.rotate(tf.image.random_flip_left_right(image), angles=random.uniform(0, rads)), level=random.uniform(-0.1, 0.1), replace=0), label), tf.data.experimental.AUTOTUNE)
  dataset = dataset.map(lambda image, label: (_mean_image_subtraction(image, CHANNEL_MEANS, NUM_CHANNELS), label), tf.data.experimental.AUTOTUNE) #, keep_position=True)
  ...
  #dataset = dataset.map(_muyu_process, tf.data.experimental.AUTOTUNE)

  if DISPATCHER_IP is not None and is_training is True and DISTRIBUTE_CHOICE == 2:
    dataset = dataset.prefetch(buffer_size=tf.data.experimental.AUTOTUNE)
    dataset = dataset.apply(tf.data.experimental.service.distribute(
      processing_mode="distributed_epoch", service="grpc://" + DISPATCHER_IP + ":31000",
      max_outstanding_requests=16, max_request_pipelining_per_worker=2, compression=None,
  ...
  #dataset = dataset.map(_muyu_post_process, tf.data.experimental.AUTOTUNE)

  if DISPATCHER_IP is not None and is_training is True and DISTRIBUTE_CHOICE == 3:
    dataset = dataset.prefetch(buffer_size=tf.data.experimental.AUTOTUNE)
    dataset = dataset.apply(tf.data.experimental.service.distribute(
      processing_mode="distributed_epoch", service="grpc://" + DISPATCHER_IP + ":31000",
      max_outstanding_requests=32, max_request_pipelining_per_worker=2, compression=None,job_name="resnet", fast_flow_offloading=9,
    ))
    
  dataset = dataset.batch(batch_size, drop_remainder=drop_remainder)

  if DISPATCHER_IP is not None and is_training is True and DISTRIBUTE_CHOICE == 35:
    dataset = dataset.prefetch(buffer_size=tf.data.experimental.AUTOTUNE)
    dataset = dataset.apply(tf.data.experimental.service.distribute(
      processing_mode="distributed_epoch", service="grpc://" + DISPATCHER_IP + ":31000",
      max_outstanding_requests=32, max_request_pipelining_per_worker=2, compression=None,job_name="resnet", fast_flow_offloading=8,
    ))
  ...
  if DISPATCHER_IP is not None and is_training is True and DISTRIBUTE_CHOICE == 4 and not parallel_reads: # and DISPATCHER_IP !='loc':
    # dataset = dataset.prefetch(buffer_size=tf.data.experimental.AUTOTUNE)
  ...
  if parallel_reads:
    datasets = [make_distributed() for _ in range(num_parallel_reads)]
  ...
  #if DISPATCHER_IP == 'loc' and is_training is True and DISTRIBUTE_CHOICE == 4:
    '''dispatcher = tf.data.experimental.service.DispatchServer()
    dispatcher_address = dispatcher.target.split("://")[1]
    print(dispatcher_address)
    workers = []
    for i in range(4):
        #workers.append(tf.data.experimental.service.WorkerServer(
        #port=i, dispatcher_address=dispatcher_address))
        workers.append(tf.data.experimental.service.WorkerServer(
...
def _decode_and_crop(dataset):
    image_buffer, label, bbox = parse_example_proto(dataset)
...
def _muyu_process(raw_record):
    image_buffer, label, bbox = parse_example_proto(raw_record)
    image = _decode_crop_and_flip(image_buffer, bbox, NUM_CHANNELS)
    #image = _resize_image(image, DEFAULT_IMAGE_SIZE, DEFAULT_IMAGE_SIZE)
    return image, label

def _muyu_post_process(image, label):

    #image = _resize_image(image, DEFAULT_IMAGE_SIZE, DEFAULT_IMAGE_SIZE)
    image = _resize_image(image, DEFAULT_IMAGE_SIZE, DEFAULT_IMAGE_SIZE)
    image = tf.image.random_flip_left_right(image)
    rads = 5 * math.pi / 180
    image = tfa.image.rotate(image, angles=random.uniform(0, rads))
    #image = tfa.image.equalize(image)
    image = tfa.image.shear_x(image, level=random.uniform(-0.1, 0.1), replace=0)
    #image = _resize_image(image, DEFAULT_IMAGE_SIZE, DEFAULT_IMAGE_SIZE)
    #image.set_shape([DEFAULT_IMAGE_SIZE, DEFAULT_IMAGE_SIZE, NUM_CHANNELS])

    image = _mean_image_subtraction(image, CHANNEL_MEANS, NUM_CHANNELS)

    image = tf.cast(image, tf.dtypes.float16)
...
def input_fn(is_training,
  ...
  if DISPATCHER_IP is not None and is_training is True and DISTRIBUTE_CHOICE == 0:
    dataset = dataset.apply(tf.data.experimental.service.distribute(
      processing_mode="distributed_epoch", service="grpc://" + DISPATCHER_IP + ":31000",
      max_outstanding_requests=16, max_request_pipelining_per_worker=2, compression=None,
        job_name="resnet"
