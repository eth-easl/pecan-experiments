...
flags.DEFINE_integer(name='num_local_workers', default=0,
                     help='Specify number of local workers')
flags.DEFINE_string(name='dispatcher_ip', default="otmraz-cachew-dispatcher-bl9v",
                     help='specify dispatcher ip')
...
def main(_):
    ...
    print(flags.FLAGS.num_local_workers)

    '''dispatcher = tf.data.experimental.service.DispatchServer()
    dispatcher_address = dispatcher.target.split("://")[1]
    full_disp_address = dispatcher.target
    with open("disp_address.txt", "w") as text_file:
        text_file.write(full_disp_address)
    print(dispatcher_address)
    workers = []
    for i in range(flags.FLAGS.num_local_workers):
    ...
    if DISPATCHER_IP=='None':
        DISPATCHER_IP=None
    DISP_IP = get_disp_ip(DISPATCHER_IP)
    print("Dispatcher name is: " + str(DISP_IP))
    loc_workers = tf.data.experimental.service.spawn_loc_workers(workers=flags.FLAGS.num_local_workers, dispatcher=str(DISP_IP)+":31000")
    #spawn_local_worker(
    #    flags.FLAGS.num_local_workers,
    #    f"{flags.FLAGS.dispatcher_ip}:31000"
    #)
...
if __name__ == '__main__':
    ...
    #tf.profiler.experimental.server.start(6011)

    #tf.profiler.experimental.client.trace('grpc://localhost:6009','dump', 2000)
    #tf.profiler.experimental.start('dump')
    app.run(main)
    #tf.profiler.experimental.client.trace('grpc://localhost:6011','dump', 2000)
