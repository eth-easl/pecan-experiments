...
exp_root="$project_root/results/TPU_local"
exp_profile_dir="Apr3_TPU_local_small"
...
for remote_worker_count in 1 2
do
...
#  profile_log_path="gs://muyu-kops-store/$exp_profile_dir/$exp_name"
...
#  python capture_profile.py $profile_log_path $local_worker_count $remote_workers > $profile_path &
  ...
  echo bash run_imageNet_TPU.sh $exp_name $main_program_log_path $local_worker_count $dispatcher_name
  bash run_imageNet_TPU.sh $exp_name $main_program_log_path $local_worker_count $dispatcher_name
